...
export const App: React.FC = () => {
  ...
  const handleSelectZoomDir = async () => {
    const result = await window.zoomDuo.invoke<SelectDirectoryResult>("zoom/select-recording-dir");
    if (!result || result.canceled) {
      return;
    }

    try {
      const updated = await window.zoomDuo.invoke<AppConfig>("config/update", {
        key: "zoom_recording_dir",
        value: result.path ?? ""
      });
      setConfig(updated);
  ...
  const handleSelectCsv = async () => {
    const result = await window.zoomDuo.invoke<ThemeSelectResult>("themes/select-csv");
    if (!result || result.canceled) {
      return;
    }
    ...
    try {
      ...
      if (result?.status === "ok") {
        ...
        setLastConsent({
          ...
          sessionDir: consentConfig.last_session_dir,
          path: consentConfig.last_pdf_path
        });
        setConsentVisible(false);
      } else {
        setConsentError(result?.message ?? "同意 PDF の生成に失敗しました。");
      }
    } catch (error) {
      setConsentError(
  ...
  }, [status.state, sessionState.running, sessionState.theme, availableThemes, config?.session?.talk_seconds, startSession, stopSession, setSessionTheme]);
  return (
    <>
      <main style={{ padding: "24px", fontFamily: "sans-serif", position: "relative", color: "#f5f5f5" }}>
        ...
        <section>
          ...
          <div style={{ marginTop: "12px" }}>
            ...
            <span style={{ color: "#ddd" }}>
              {config?.zoom_recording_dir || "未設定"}
            </span>
          </div>
          {monitorMessage && (
            <p style={{ color: "#9fd7ff", marginTop: "8px" }}>{monitorMessage}</p>
          )}
          {lastConsent && (
            <p style={{ color: "#9fd7ff", marginTop: "8px" }}>
              直近の同意: {lastConsent.timestamp ? new Date(lastConsent.timestamp).toLocaleString() : "未記録"} / セッションフォルダ:{" "}
              {lastConsent.sessionDir ?? "不明"}{lastConsent.path ? ` (PDF: ${lastConsent.path})` : ""}
            </p>
          )}
          <button
        ...
        <section>
          ...
          >
            {config ? JSON.stringify(config, null, 2) : "読み込み中..."}
          </pre>
        </section>
